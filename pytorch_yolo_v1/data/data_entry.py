...
from data.dataset_base import detection_collate
...
def select_train_loader(args):
    ...
    train_loader = DataLoader(train_dataset, args.batch_size, shuffle=True, collate_fn=detection_collate, num_workers=4, pin_memory=True, drop_last=False)
...
def select_eval_loader(args):
    ...
    # setting the output type of the dataloader
    val_loader = DataLoader(eval_dataset, args.batch_size, shuffle=False, collate_fn=detection_collate, num_workers=4, pin_memory=True, drop_last=False)
    return val_loader
